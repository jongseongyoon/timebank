...
const schema = z.object({
  ...
  tpAmount: z.number().positive().max(100),
  note: z.string().max(200).optional(),
})

// QR 스캔을 통한 TP 직접 송금 (제공자 → 수신자)
export async function POST(req: NextRequest) {
  ...
  const { receiverId, tpAmount, note } = parsed.data
  ...
  if (Number(sender.tpBalance) < tpAmount) return NextResponse.json({ error: 'TP 잔액 부족' }, { status: 400 })
  ...
  const txHash = crypto.createHash('sha256')
    .update(`qr-transfer-${senderId}-${receiverId}-${tpAmount}-${Date.now()}`)
    .digest('hex')
